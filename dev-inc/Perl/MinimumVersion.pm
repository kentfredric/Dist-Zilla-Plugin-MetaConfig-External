...
sub _minimum_explicit_version {
  ...
  my $explicit = $self->Document->find(
    sub {
...
sub _minimum_syntax_version {
  ...
  my @rules = sort { $CHECKS{$b} <=> $CHECKS{$a} } grep { not( exists $checks2skip{$_} ) and $CHECKS{$_} > $filter } keys %CHECKS;
...
sub _regex {
  ...
  shift->Document->find(
    sub {
      return ''
        unless grep { $_[1]->isa($_) } qw/PPI::Token::QuoteLike::Regexp PPI::Token::Regexp::Match PPI::Token::Regexp::Substitute/;
  ...
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and $MATCHES{_perl_5012_pragmas}->{ $_[1]->pragma };
    }
  );
}
...
sub _sort_subref {
  shift->Document->find_first(
    sub {
      ...
      $e1->isa('PPI::Token::Whitespace') or return '';
      $e1 = $e1->next_sibling;
           $e1->isa('PPI::Token::Word')
        or $e1->isa('PPI::Token::Symbol')
        or $e1->isa('PPI::Token::Cast')
        or $e1->isa('PPI::Structure::List')
        or return '';
      return 1;
    }
  );
...
sub _perl_5010_pragmas {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and $MATCHES{_perl_5010_pragmas}->{ $_[1]->pragma };
    }
  );
}
  ...
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Token::Operator')
        and $MATCHES{_perl_5010_operators}->{ $_[1]->content };
    }
  );
}
  ...
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Token::Magic')
        and $MATCHES{_perl_5010_magic}->{ $_[1]->symbol };
    }
  );
}
...
sub _perl_5008_pragmas {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and $MATCHES{_perl_5008_pragmas}->{ $_[1]->pragma };
    }
  );
}
...
sub _bugfix_magic_errno {
  ...
  my $element  = $Document->find_first(
    sub {
      $_[1]->isa('PPI::Token::Magic')
        and $_[1]->symbol eq '$^E';
    }
...
sub _pragma_utf8 {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and ( ( $_[1]->module and $_[1]->module eq 'utf8' )
        or ( $_[1]->pragma and $_[1]->pragma eq 'utf8' ) )
...
sub _constant_hash {
  shift->Document->find_first(
    sub {
        ...
        and $_[1]->type
        and $_[1]->type eq 'use'
        and $_[1]->module eq 'constant'
        and $_[1]->schild(2)->isa('PPI::Structure');
    }
  );
}
...
sub _perl_5006_pragmas {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and $MATCHES{_perl_5006_pragmas}->{ $_[1]->pragma };
    }
  );
}
...
sub _perl_5005_pragmas {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        and $MATCHES{_perl_5005_pragmas}->{ $_[1]->pragma };
    }
  );
}
  ...
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Statement::Include')
        ...
        or (  $_[1]->module =~ /\bException\b/
          and $_[1]->module !~ /^(?:CPAN)::/ )
        or $_[1]->module =~ /\bThread\b/
        or $_[1]->module =~ /^Error\b/
        or $_[1]->module eq 'base'
        or $_[1]->module eq 'Errno'
        );
    }
  );
}
...
sub _weaken {
  shift->Document->find_first(
    sub {
      (       $_[1]->isa('PPI::Statement::Include')
          and $_[1]->module eq 'Scalar::Util'
          and $_[1]->content =~ /[^:]\b(?:weaken|isweak)\b[^:]/ )
        or (
        $_[1]->isa('PPI::Token::Word')
        and ($_[1]->content eq 'Scalar::Util::isweak'
          or $_[1]->content eq 'Scalar::Util::weaken' )
...
sub _5005_variables {
  shift->Document->find_first(
    sub {
      $_[1]->isa('PPI::Token::Magic')
        and ( $_[1]->symbol eq '$!' or $_[1]->symbol eq '$^R' );
    }
  );
}
...
sub _SELF {
  ...
  if ( _CLASS( $param->[0] )
    and $param->[0]->isa('Perl::MinimumVersion') )
  {
...
sub _max {
  ...
  my @valid =
    map  { [ $_, $_->isa('Perl::MinimumVersion::Reason') ? $_->version : $_ ] }
    grep { _INSTANCE( $_, 'Perl::MinimumVersion::Reason' ) or _INSTANCE( $_, 'version' ) } @_
    or return '';
