...
sub __as_pod_string_begin {
  ...
  $string .= sprintf "=%s %s", $self->closing_command, $colon . $self->format_name;
...
sub __as_pod_string_for {
  ...
  my $string = sprintf "=for %s %s", $colon . $self->format_name, $self->children->[0]->as_pod_string;
...
sub as_debug_string {
  ...
  my $string = sprintf "=%s %s", $self->command, $colon . $self->format_name;
