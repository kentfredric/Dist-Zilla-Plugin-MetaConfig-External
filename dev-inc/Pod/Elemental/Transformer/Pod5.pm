...
sub _upgrade_nonpod {
  ...
  @$in_paras =
    map { $_->isa( $self->_gen_class('Nonpod') ) ? $self->_class('Nonpod')->new( { content => $_->content, } ) : $_ } @$in_paras;
}
...
PARA: while ( my $para = shift @{$in_paras} ) {
    ...
    if ( $para->command eq 'for' ) {
      ...
      my $region = $region_class->new(
        {
          children    => [ $self->_gen_class('Text')->new( { content => $content } ), ],
          format_name => $target,
          is_pod      => $colon ? 1 : 0,
          content     => "\n",
        }
...
sub _collect_runs {
  ...
  $self->_collect_runs( $_->children ) foreach grep { $_->isa( $self->_class('Region') ) } @$paras;
...
PASS: for my $start ( 0 .. $#$paras ) {
  ...
  NEXT: for my $next ( $start + 1 .. $#$paras ) {
    ...
    splice @$paras, $start, scalar(@to_collect),
      $class->new(
      {
        content => $new_content,
      }
