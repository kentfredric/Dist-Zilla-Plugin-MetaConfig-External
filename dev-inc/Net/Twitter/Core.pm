...
sub _prepare_request {
  ...
  if ( $http_method eq 'PUT' ) {
    ...
    if ( $content_type && $content_type eq 'application/json' ) {
      ...
      $msg =
        ( first { ref } values %natural_args )
        ? POST(
        $uri,
        Content_Type => 'form-data',
        Content      => [ map { ref $_ ? $_ : encode_utf8 $_ } %natural_args, ],
        )
...
sub _since_as_datetime {
  ...
  return
    ...
    || croak "Invalid 'since' parameter: $since. Must be a DateTime, epoch, string in Twitter timestamp format, or YYYY-MM-DD.";
}
