...

{
  ...
  coerce(
    ...
    from ArrayRef,
    via {
      to_Paths( [ grep { length } @$_ ] )
    },
    from Any,
    via { length($_) ? to_Paths($_) : [] },
  );
...

{
  ...
  coerce $type, from ArrayRef [Str], via {
    [ map { qr/$_/ } @$_ ]
  };
...
around dump_config => sub {
  ...
  $config->{ +__PACKAGE__ } =
    { ( map { $_ => [ sort @{ $self->$_ } ] } qw(allow_dirty allow_dirty_match) ), changelog => $self->changelog, };
...
sub list_dirty_files {
  ...
  if ( $git_root ne '.' ) {
    ...
    $git_root = Path::Tiny::path($git_root)->absolute($dzil_root)->realpath;
    ...
    for my $fn (@filenames) {
      try {
        $fn = Path::Tiny::path($fn)->absolute($dzil_root)->realpath    # process ..
          ->relative($git_root)->stringify;
      };
    }
  ...
  return grep { /$allowed/ ? $listAllowed : !$listAllowed } $git->ls_files( { modified => 1, deleted => 1 } );
