...
my %NOT_INDEXED = map { ; $_ => 1 } qw(Config DB Errno integer NEXT perl Pod::Functions);
...
PKG: for my $pkg ( sort @modules ) {
  ...
  if ( keys %unmet ) {
    for my $pkg ( sort keys %unmet ) {
      $self->log(
        [ "you required %s version %s but CPAN only has version %s", $pkg, "$unmet{$pkg}{required}", "$unmet{$pkg}{indexed}", ] );
    }
  }

  return if $self->zilla->chrome->prompt_yn( "release despite missing prereqs?", { default => 0 } );
