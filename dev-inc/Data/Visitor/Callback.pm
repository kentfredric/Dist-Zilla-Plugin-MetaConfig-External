  ...
  my @class_callbacks = do {
    ...
    grep {
      ...
      m{ :: | ^[A-Z] }x               # if it looks kinda lack a class name
        or scalar keys %{"${_}::"}    # or it really is a class
    } keys %callbacks;
  };
  ...
  for my $data (@_) {
    ...
    if ( $refaddr and exists $replaced_hash->{$refaddr} ) {
      if (FIVE_EIGHT) {
        ...
        carp( q{Assignment of replacement value for already seen reference }
            . overload::StrVal($data)
            . q{ to container doesn't work on Perls older than 5.8, structure shape may have lost integrity.} );
      }
    }
